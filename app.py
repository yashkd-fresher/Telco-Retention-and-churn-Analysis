...
# key factors that contribute to customer retention or attrition.
# """)
...
#     # Replace 'Yes'/'No' with 1/0 for Churn
...
#     # Visualize relationship between categorical features and churn
...
#     # Visualize relationship between numerical features and churn
...
#         opacity=0.7,
#         barmode='overlay',
#         title=f'Distribution of {selected_num_col} by Churn Status',
...
#     # Create a copy of data with churn as numeric
#     data_corr = data.copy()
...
#                     # Ensure we're only transforming values that the encoder knows
#                     try:
#                         input_processed[col] = le.transform(input_processed[col])
#                     except ValueError:
#                         # If there's a value the encoder hasn't seen, default to the most common value
...
#             shap_values = shap_values[0, :-1]  # Remove the bias term
...
#                 1. **Offer Upgrade/Discount:** Consider offering a service upgrade or discount to incentivize the customer to stay.
#                 2. **Contract Lock-In:** Suggest a longer-term contract with better benefits.
#                 3. **Customer Service Call:** Schedule a customer service call to address any concerns.
#                 4. **Personalized Retention Package:** Develop a personalized retention package based on their usage patterns.
#                 """)
#             else:
#                 st.markdown("""
#                 ### Customer Nurturing Strategies:
                
#                 1. **Cross-Selling Opportunities:** Identify services that could enhance the customer's experience.
#                 2. **Loyalty Recognition:** Acknowledge their loyalty with a small token of appreciation.
#                 3. **Service Review:** Schedule a regular service review to ensure continued satisfaction.
#                 4. **Referral Program:** Introduce a referral program to leverage their positive experience.
#                 """)
    
#     except FileNotFoundError:
#         st.error("Model file not found. Please go to the 'Model Training & Evaluation' page and train the model first.")
#     except Exception as e:
#         st.error(f"An error occurred while loading the model: {str(e)}")

# # Footer
# st.markdown("---")
# st.markdown("Telecom Customer Churn Analysis Dashboard | Created with Streamlit")
import pandas as pd
import numpy as np
import streamlit as st
import matplotlib.pyplot as plt
